...
"externalAdReply": {
      ...
      const demoteAdmin = async function(to, target=[]){
          ...
          if (!packname) packname = 'Dibuat Oleh'; if (!author) author = '© ₳₦ĐⱧł₭₳ Ø₣₣ł₵ł₳Ⱡ';author = author.replace(/[^a-zA-Z0-9]/g, '');	
     ...
     const time2 = moment().tz('Asia/Jakarta').format('HH:mm:ss')
       ...
       console.error(err)
        ...
        const ftoko = {key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "16505434800@s.whatsapp.net" } : {})},message: {"productMessage": {"product": {"productImage":{"mimetype": "image/jpeg","jpegThumbnail": fs.readFileSync(`./media/foto/thumb.jpg`)},"title": `© ʙʏ ᴀʙx-ꜱᴛᴏʀᴇ`,"description": "Andhika Official", "currencyCode": "IDR","priceAmount1000": "999999","retailerId": "₳₦ĐⱧł₭₳ Ø₣₣ł₵ł₳Ⱡ","productImageCount": 1},"businessOwnerJid": `0@s.whatsapp.net`}}}
...
danzrun = process.uptime() 
           Ramdani.setStatus(`${namabot} Aktif Selama ${(danzrun)} © Creator By ₳₦ĐⱧł₭₳ Ø₣₣ł₵ł₳Ⱡ`).catch((_)=>_); //DI UBAH YATIMM
...
> 532 ${prefix}nsfw_avatar`
               ...
               buttonsMessage = {
               contentText: `${allmenu}`,
               footerText: '© ʙʏ ᴀʙx-ꜱᴛᴏʀᴇ', imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
...
if (isBanned) return freply(mess.banned)
freply(`━━━━━ *INFO BOT* ━━━━━
❏ BASE ORI : ₳₦ĐⱧł₭₳ Ø₣₣ł₵ł₳Ⱡ
❏ TYPE SC : nodejs
❏ DI BUAT PADA : sel, 29, mart, 2022
❏ GITHUB CREATOR : https://github.com/AndhikaGanz-404
❏ YT CREATOR :-
*━━━━━━━━━━━━━━━━━━━━*`)
randommusic = fs.readFileSync(`./media/audio/${randomaudio}.mp3`)
Ramdani.sendMessage(from, randommusic, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
...
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
freply(`*SILAHLAN HUBUNGI*\nhttp://wa.me/62895359869547?text=bang+mau+order+jasa+run+heroku:v`)
break
//JASA UP HEROKU
case 'jasaupgithub':
case 'jasaup':
if (!isRegist) return freply(mess.regist)
if (isBanned) return freply(mess.banned)
freply(`*SILAHLAN HUBUNGI*\nhttp://wa.me/62895359869547?text=bang+mau+order+jasa+up+github:v`)
break
//STORE MENU//
//TOP UP GAME
case 'topupgame2':
...
case 'tiktok':
              ...
              buttonsMessage = {footerText:'Jangan Lupa Subscribe Yt ₳₦ĐⱧł₭₳ Ø₣₣ł₵ł₳Ⱡ\n Help 1k subscriber', imageMessage: imageMsg,
       ...
       case 'img2url':
         ...
         case 'asupan':
              ...
              .then(async (body) => {
               ...
               sendMediaURL(from, `http://sansekai.my.id/ptl_repost/${asupanx}`, '\`\`\`ASUPAN BRO GUA DAPAT DARI ₳₦ĐⱧł₭₳ Ø₣₣ł₵ł₳Ⱡ\`\`\`')
...
case 'ttp':
              if (!isRegist) return freply(mess.regist)
              if (isBanned) return freply(mess.banned)
              if (args.length == 0) return freply(`Example: ${prefix + command} ₳₦ĐⱧł₭₳ Ø₣₣ł₵ł₳Ⱡ`)
              woy = args.join(" ")
              freply(mess.wait)
...
Ramdani.sendMessage(from, tapib1, document, { quoted: mek, filename:`₳₦ĐⱧł₭₳ Ø₣₣ł₵ł₳Ⱡ ~ 404 ${danz}.mp3`, mimetype: 'audio/application' })
...
"groupInviteMessage": {
"groupJid": "62895359869547-1328272333@g.us",
"inviteCode": "wFHwtOxGQN8OwK2x",
"inviteExpiration": "162533333338378",
"groupName": `₳₦ĐⱧł₭₳ Ø₣₣ł₵ł₳Ⱡ ~ 404 ${danz}`,
...
"businessOwnerJid": "62895359869547@s.whatsapp.net",
"contextInfo": {
"forwardingScore": 3,
"isForwarded": true
}
...
Ramdani.sendMessage(_.jid, 
      ...
      case 'broadcastgroup': 
             ...
             for (let _ of anu) {
Ramdani.sendMessage(_.jid, 
			{"contentText": `*「 SIARAN ʙʏ ᴀʙx-ꜱᴛᴏʀᴇ」*\n*Izin Siaran Ngap:v*\n*Pesan :* ${body.slice(4)}`,
			"footerText": '© ʙʏ ʀᴀᴍᴅᴀɴɪ ᴏғғɪᴄɪᴀʟ',
			"buttons": [
			{"buttonId": `${prefix}menu`,
			"buttonText": {"displayText": "MENU"
...
//------------------< Lainnya >-------------------
      ...
      case 'caripesan':
       ...
       case 'bugreport':
              ...
              Ramdani.sendMessage('62895359869547@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'readall':
              if (!isRegist) return freply(mess.regist)
              if (isBanned) return freply(mess.banned)
